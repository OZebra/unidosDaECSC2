...
"""
José Brandão - JCCB - Agentes autônomos

1 - Aumento do número de centros de comando para que a IA expanda mais
2 - Aumento da taxa de distribuição dos trabalhadores para diminuir a chance de termos trabalhadores ociosos
3 - Aumento do threshold de vida que faz o reaper recuar para que as unidades sejam mais conservadoras nos ataques.
4 - Diminuição da distância de recuo caso um inimigo se aproxime, para que o reaper ataque por mais tempo (em tese compensado por '3')
...
class MassReaperBot(BotAI):
    ...
    async def on_step(self, iteration):
        ...
        if (
            1 <= self.townhalls.amount < 5 and self.already_pending(UnitTypeId.COMMANDCENTER) == 0
        ...
        for r in self.units(UnitTypeId.REAPER):
            ...
            if r.health_percentage < 4 / 5 and enemyThreatsClose:
            ...
            enemyThreatsVeryClose: Units = enemies.filter(
                lambda unit: unit.can_attack_ground and unit.distance_to(r) < 2.5
